...
def rgb2xyz(rgb): # rgb from [0,1]
    ...
    mask = (rgb > .04045).to(dtype=rgb.dtype, device=rgb.device)
...
def xyz2rgb(xyz):
    ...
    mask = (rgb > .0031308).to(dtype=rgb.dtype, device=rgb.device)
...
def xyz2lab(xyz):
    ...
    sc = torch.tensor((0.95047, 1., 1.08883), device=xyz.device, dtype=xyz.dtype)[None,:,None,None]

    xyz_scale = xyz/sc

    mask = (xyz_scale > .008856).to(dtype=xyz.dtype, device=xyz.device)
...
def lab2xyz(lab):
    ...
    z_int = torch.max(torch.tensor((0,), device=lab.device, dtype=lab.dtype), z_int)

    out = torch.cat((x_int[:,None,:,:],y_int[:,None,:,:],z_int[:,None,:,:]),dim=1)
    mask = (out > .2068966).to(dtype=out.dtype, device=out.device)

    out = (out**3.)*mask + (out - 16./116.)/7.787*(1-mask)

    sc = torch.tensor((0.95047, 1., 1.08883), device=out.device, dtype=out.dtype)[None,:,None,None]
...
def decode_ind_ab(data_q, opt):
    ...
    data_ab = ((data_ab.to(device=data_q.device, dtype=data_q.dtype)*opt.ab_quant) - opt.ab_max)/opt.ab_norm
...
def decode_mean(data_ab_quant, opt):
    ...
    a_range = torch.arange(-opt.ab_max, opt.ab_max + 1e-5, step=opt.ab_quant, device=data_ab_quant.device, dtype=data_ab_quant.dtype)[None,:,None,None]
